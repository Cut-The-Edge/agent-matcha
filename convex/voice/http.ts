...
export const logVoiceUsageHandler = httpAction(async (ctx, request) => {
  ...
  const {
    ...
    agentTokens,
    transcriptSegments,
    // ElevenLabs actual cost (from post-call webhook)
    elevenlabsCostUsd,
    elevenlabsMinutes,
  } = body;

  const durationMinutes = (durationSecs || 0) / 60;

  // If we have actual ElevenLabs cost from the webhook, use that
  // Otherwise fall back to per-component estimates
  if (elevenlabsCostUsd !== undefined) {
    // Single log entry with actual ElevenLabs bundled cost
    await ctx.runMutation(internal.analytics.tokenTracking.logTokenUsage, {
      ...
      outputTokens: 0,
      totalTokens: 0,
      costUsd: Math.round((elevenlabsCostUsd || 0) * 1_000_000) / 1_000_000,
      latencyMs: (durationSecs || 0) * 1000,
      entityType: "call",
      entityId: callId,
      metadata: {
        service: "elevenlabs",
        ...
        transcriptSegments,
        llmModel: llmModel || "gpt-4.1-mini",
        ttsModel: ttsModel || "eleven_flash_v2",
        sttModel: sttModel || "elevenlabs/scribe",
        actualCostUsd: elevenlabsCostUsd,
      },
    });
    console.log("[logUsage] ElevenLabs: $%s for %s min | callId=%s",
      (elevenlabsCostUsd || 0).toFixed(4), durationMinutes.toFixed(1), callId);
  } else {
    // Legacy: per-component cost estimates (for old LiveKit calls)
    const DEEPGRAM_COST_PER_MIN = 0.0043;
    const ELEVENLABS_COST_PER_CHAR = 0.00003;

    const sttCost = durationMinutes * DEEPGRAM_COST_PER_MIN;
    await ctx.runMutation(internal.analytics.tokenTracking.logTokenUsage, {
      processType: "voice-intake",
      provider: "other",
